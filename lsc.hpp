...
template <typename T, int subGroupSize,
         std::enable_if<std::is_scalar<T>::value, bool>::type = true>
static inline void lscLoad(T& var, void *addr) {
  LscLoad<sizeof(T), 1, subGroupSize>::run<T>(var, addr);
}

template <typename T, int subGroupSize,
         std::enable_if<std::is_scalar<T>::value, bool>::type = true>
static inline void lscStore(void *addr, const T& var) {
  LscStore<sizeof(T), 1, subGroupSize>::run<T>(addr, var);
}
...
//
template <typename T, int subGroupSize,
         std::enable_if<std::is_compound<T>::value, bool>::type = true>
static inline void lscLoad(T& var, void *addr) {
  LscLoad<sizeof(T::value_type), T::size(), subGroupSize>::run<T>(var, addr);
}

template <typename T, int subGroupSize,
         std::enable_if<std::is_compound<T>::value, bool>::type = true>
static inline void lscStore(void *addr, const T& var) {
  LscStore<sizeof(T::value_type), T::size(), subGroupSize>::run<T>(addr, var);
}

// Intended usage:
//    lscLoad<16, DataShuffle::none>(array, adrs, 1024, 1024, 4096, 0, 128);

template <T, int Row, int Width, DataShuffle Transpose>
static inline void lscLoad(T (&array) [Row], void* SurfaceBase,
    int SurfaceWidth, int SurfaceHeight, int SurfacePitch,
    int Src0AddrX, int Src0AddrY) {
  Lsc2DLoad<Row, Width, Transpose>::run<T>(
      array, SurfaceBase,
