...
template <typename T, typename CastT, int BH, int BW, int SG_SZ>
struct tileCastAdd {
  tileCastAdd(T* dst, const T* src,
      int surfaceH, int surfaceW, int surfaceP
  ): src(src), dst(dst), surfaceH(surfaceH), surfaceW(surfaceW), surfaceP(surfaceP)
  {}

  using tileT = __ArrayMatrix<T, BH, BW, DataShuffle::none, SG_SZ>;
...
template <typename T, int BH, int BW, int SG_SZ>
struct tileHSplitAdd {
  tileHSplitAdd(T* dst, const T* src,
      int surfaceH, int surfaceW, int surfaceP
  ): src(src), dst(dst), surfaceH(surfaceH), surfaceW(surfaceW), surfaceP(surfaceP)
  {}

  using tileT = __ArrayMatrix<T, 2 * BH, BW, DataShuffle::none, SG_SZ>;
