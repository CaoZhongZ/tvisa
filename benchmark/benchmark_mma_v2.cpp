...
template <typename T> struct MMAKernelImpl {
  ...
  operator()(sycl::nd_item<3> item) const {
    ...
    constexpr const int mma_m = MMA32x32x32::mma_m_in_elem;
    constexpr const int mma_n = MMA32x32x32::mma_n_in_elem;
    constexpr const int mma_k = MMA32x32x32::mma_k_in_bytes / sizeof(T);
    ...
    __ArrayMatrix<T, mma_m, mma_n / 2, DataShuffle::none, sg_size>
        acc[segment_m][segment_n][2];
    ...
    AddressPayload<mma_k, mma_n / 2, 2> b_block_address[segment_k][segment_n];
    ...
    for (int k = 0; k < k_loop; ++k) {
      ...
      // load B
      __ArrayMatrix<T, mma_k, mma_n / 2, DataShuffle::vnni, sg_size, 2>
      ...
      for (int ik = 0; ik < segment_k; ++ik) {
        for (int im = 0; im < segment_m; ++im) {
          for (int in = 0; in < segment_n; ++in) {
            dpas<32, 16, 32>(acc[im][in], mat_a[im][ik], mat_b[ik][in]);
          }
        }
      }
      asm("fence_sw");
    ...
    // store c
    AddressPayload<mma_m, mma_n / 2> c_address(
    ...
    for (int im = 0; im < segment_m; ++im) {
      for (int in = 0; in < segment_n; ++in) {
        for (int ii = 0; ii < 2; ++ii) {
          AddressPayload<mma_m, mma_n / 2> c_block_address(c_address);
          c_block_address.addSrc0AddrY(im * mma_m);
          c_block_address.addSrc0AddrX(in * mma_n + ii * 16);
          lscStore<CacheCtrl::L1WB_L3WB>(c_block_address, acc[im][in][ii]);
        }
      }
    }
#else
